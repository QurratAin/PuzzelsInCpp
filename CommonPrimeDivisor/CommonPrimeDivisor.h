...
N = 15 and M = 75, the prime divisors are the same:
{3, 5};
N = 10 and M = 30, the prime divisors aren't the same: 
{2, 5} is not equal to {2, 3, 5};
N = 9 and M = 5, the prime divisors aren't the same: 
{3} is not equal to {5}.
Write a function:
...
For example, given:
    ...
    A[1] = 10   B[1] = 30
    A[2] = 3    B[2] = 5
the function should return 1, because only one pair 
(15, 75) has the same set of prime divisors.
*/
class CommonPrimeDivisor
{
private:
    unsigned int gcd(int x, int y);
    unsigned long int hasCommon(unsigned long int num, unsigned long int factor);
public:
	int solution(vector<int>& A, vector<int>& B);
};
